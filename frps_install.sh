...
# 显示引导文本
echo -e "${Red}============首富虎哥一键挖矿脚本======================${Font}"
echo -e "${Red}首富频道出品，仅供会员学习使用，请勿用于商业用途${Font}"
echo -e "${Red}首富Youtube频道: https://www.youtube.com/@shoufu${Font}"
echo -e "${Red}加入首富电报财富群: https://t.me/shoufuhuge 一起暴富${Font}"
echo -e "${Red}=======================================================${Font}"
...
# download
echo -e "${Green}正在查找 frp 下载地址...${Font}"  # 提示用户正在查找下载地址
...
sudo mv ${FILE_NAME}/${FRP_NAME} ${FRP_PATH}
sudo mv ${FRP_NAME}.toml ${FRP_PATH}

# 配置 frps.service
sudo bash -c 'cat >/lib/systemd/system/frps.service <<EOF
[Unit]
